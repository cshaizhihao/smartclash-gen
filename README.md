...
### v0.13.41
- 开始进入安装体验优化阶段：安装脚本全汉化，并补首次安装时的交互式端口输入
- 默认不传 `--port` 时，安装脚本会提示用户手动输入 mixed-port，直接回车则使用 7892
- 安装完成后的输出说明也同步改成正式中文表达

### v0.13.40
- 开始进入下一阶段：管理员首开流程回归，首次打开将先进入管理员账号密码设置逻辑，再进入开屏动画与主界面
- 认证入口文案同步正式化，避免继续保留测试阶段的“开始使用”表达
- 维持每轮同步升级可见版本号与静态资源 query 的规则

### v0.13.39
- 最终验收继续清残余：修掉页面里最后一条仍写成 `YAML + Markdown` 的用户可见提示，统一为真实能力 `YAML 下载 + 订阅链接`
- 这一轮作为最终验收中的一致性收口，不扩新功能
